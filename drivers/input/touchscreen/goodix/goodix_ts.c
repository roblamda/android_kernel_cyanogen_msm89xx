...
#if defined(CONFIG_TOUCHSCREEN_GT9XX_GESTURE)
static int set_gesture_switch(struct device *dev, bool enable)
{
   struct goodix_ts_data *data = dev_get_drvdata(dev);

   data->gesture_wakeup_enable = enable;
   if(enable == 0)
   		disable_irq_wake(data->client->irq);
   	else enable_irq_wake(data->client->irq);

	printk("GTP %s is %d.\n",__func__,data->gesture_wakeup_enable);
   return 0;
}
...
s32 gtp_send_cfg(struct i2c_client *client)
{
    ...
    if (ts->fixed_cfg)
    {
        GTP_ERROR("Ic fixed config, no config sent!");
        return 0;
    }
    ...
    {               
        ...
        {    
			...
			if(i >= ts->pdata->gesture_key->nbuttons)
				GTP_ERROR("have no gesture to report.");
			else{
